...
class _CombinedStepState extends ConsumerState<CombinedStep> {
  ...
  void _initializeThemeControllers() {
    ...
    _themeControllers = List.generate(
      ...
      (index) =>
          TextEditingController(text: widget.state.themes[index].themeText),
    );
  ...
  void didUpdateWidget(covariant CombinedStep oldWidget) {
    ...
    if (widget.state.themes.length != oldWidget.state.themes.length) {
      ...
      for (int i = 0; i < widget.state.themes.length; i++) {
        if (widget.state.themes[i].themeText !=
            oldWidget.state.themes[i].themeText) {
          themesChanged = true;
          break;
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    final totalActionItems =
        widget.state.actionItems.where((a) => a.actionText.isNotEmpty).length;
    final completedActionItems =
        widget.state.actionItems.where((a) => a.isCompleted).length;
    final progressResult =
        totalActionItems == 0 ? 0.0 : (completedActionItems / totalActionItems);
        ...
        if (hasGoal) ...[
          Container(
            ...
            decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: isDark
                      ? const Color(0xFF333220)
                      : const Color(0xFFE6E6DB), // Border Dark/Light
                  width: 1,
                ),
                boxShadow: [
                  BoxShadow(
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        ...
                        color: isDark
                            ...
                            : const Color(0xFF8C8B5F), // Text Light Color
                      ),
                    ),
                  ],
                ),
                              ...
                              children: [
                                ...
                                Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      // Priority Selector
                                      _buildPrioritySelector(themeIndex,
                                          themeModel.priority, isDark),
                                            ...
                                            onTap: () {
                                              showCupertinoDialog(
                                                ...
                                                builder: (context) =>
                                                  ...
                                                  content: Text(
                                                      '액션타겟 ${themeIndex + 1}을(를) 삭제하시겠습니까?'),
                                                  actions: [
                                      ...
                                      _buildActionItems(themeIndex,
                                          _themeControllers[themeIndex].text),
                                    ],
                                  ),
                                ),
                              ],
    ...
    switch (priority) {
      ...
      case GoalPriority.none:
        color = isDark
            ...
            : const Color(0xFF9CA3AF); // Slate-like
        text = 'No Priority';
        break;
    }
  ...
  Widget _buildPrioritySelector(
      int themeIndex, GoalPriority current, bool isDark) {
    return Container(
      padding: const EdgeInsets.all(4),
      decoration: BoxDecoration(
      ...
      child: Row(
        children: [
          _buildPriorityOption(themeIndex, GoalPriority.high, 'High',
              Colors.red, current == GoalPriority.high),
          _buildPriorityOption(themeIndex, GoalPriority.medium, 'Medium',
              Colors.orange, current == GoalPriority.medium),
          _buildPriorityOption(themeIndex, GoalPriority.low, 'Low',
              Colors.blueGrey, current == GoalPriority.low),
        ],
      ),
    );
  }

  Widget _buildPriorityOption(int themeIndex, GoalPriority priority,
              ...
              Expanded(
                child: TextField(
                  ...
                  style: TextStyle(
                    ...
                    decoration: existing.status == ActionStatus.completed
