...
class _MandalartViewerState extends ConsumerState<MandalartViewer> {
                    ...
                    Semantics(
                      ...
                      child: CupertinoButton.filled(
                        onPressed: () {
                          ...
                          ref.read(activeThemeIndexProvider.notifier).state =
                              null;
                        },
                        child: const Row(
                          mainAxisSize: MainAxisSize.min,
                        ...
                        Semantics(
                          ...
                          child: CupertinoButton.filled(
                            onPressed: () {
                              ...
                              setState(() => currentView = 'full');
                              ref
                                  .read(activeThemeIndexProvider.notifier)
                                  .state = null;
                            },
                            child: const Row(
                              mainAxisSize: MainAxisSize.min,
          ...
          child: A4MandalartLayout(
            ...
            onToggleAction: (themeIndex, actionIndex) {
              ...
              final currentItem = widget.state.actionItems.firstWhere(
                (item) =>
                    item.themeId == 'theme-$themeIndex' &&
                    item.order == actionIndex,
  ...
  Widget _buildTodoListPreview() {
    ...
    final incompleteActions = widget.state.actionItems
        .where((action) =>
            !action.isCompleted && action.actionText.trim().isNotEmpty)
        .toList();
  ...
  Future<void> _handleWallpaperExport(
    ...
    try {
      ...
      if (isA4Only) {
        ...
        // 전체 화면 저장
        image =
            await ImageService.captureWithPreset(_screenshotController, preset);
      }
      ...
      if (isDownload) {
        if (kIsWeb) {
          ImageService.downloadWeb(
              image, ImageService.getFileNameForPreset(preset));
          if (mounted) {
      ...
      if (mounted) {
        HapticFeedback.mediumImpact();
        _showCupertinoAlert(
            '$contentType ${ImageService.getPresetLabel(preset)} 이미지가 갤러리에 저장되었습니다.');
      }
    } catch (error) {
      if (mounted) {
    ...
    if (mounted) {
      ...
      showCupertinoDialog(
        ...
        builder: (context) => CupertinoAlertDialog(
          ...
          content: const Text(
              '현재 작업 중인 만다라트를 불러온 데이터로 교체하시겠습니까?\n현재 데이터는 저장되지 않습니다.'),
          actions: [
  ...
  Future<void> _saveMandalart() async {
    try {
      ...
      final hasExisting = await ref
          .read(mandalartProvider.notifier)
          .hasMandalartWithTitle(currentTitle);
  ...
  Future<void> _showSaveAsDialog() async {
    final TextEditingController titleController =
        TextEditingController(text: widget.state.displayName.trim());
  ...
  Future<void> _performSave({required bool forceNew}) async {
    try {
      final (id, isOverwrite) = await ref
          .read(mandalartProvider.notifier)
          .saveCurrentMandalart(forceNew: forceNew);
