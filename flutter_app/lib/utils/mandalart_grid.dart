...
class GridCell {
  ...
  const GridCell({this.text, required this.type, this.isCompleted = false});
}

List<List<GridCell>> createMandalartGrid(MandalartStateModel state) {
  final grid = List.generate(9, (_) => List.generate(9, (_) => const GridCell(type: 'empty')));
  ...
  for (var i = 0; i < filledThemes.length && i < 8; i++) {
    ...
    grid[tp[0]][tp[1]] = GridCell(text: tText, type: 'theme');

    final oc = outerThemeCenters[i];
    grid[oc[0]][oc[1]] = GridCell(text: tText, type: 'outer-theme');
    ...
    for (var j = 0; j < actions.length && j < 8; j++) {
      final a = actions[j];
      final p = apos[j];
      grid[p[0]][p[1]] = GridCell(text: a.actionText, type: 'action', isCompleted: a.isCompleted);
    }
  }
...
List<List<GridCell>> createThemeGrid(MandalartStateModel state, int themeIndex) {
  final grid = List.generate(3, (_) => List.generate(3, (_) => const GridCell(type: 'empty')));
  final themes = state.themes.where((t) => t.trim().isNotEmpty).toList();
  if (themeIndex < 0 || themeIndex >= themes.length) return grid;

  grid[1][1] = GridCell(text: themes[themeIndex], type: 'theme');
